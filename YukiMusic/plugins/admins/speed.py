...
from YukiMusic.utils import AdminRightsCheck
from YukiMusic.utils.database import is_active_chat, is_nonadmin_chat
from YukiMusic.utils.decorators.language import languageCB
from YukiMusic.utils.inline import close_markup, speed_markup
...
async def playback(cli, message: Message, _, chat_id):
    ...
    return await message.reply_text(
        text=_["admin_28"].format(yuki.mention),
        reply_markup=upl,
    )


@yuki.on_callback_query(filters.regex("SpeedUP") & ~BANNED_USERS)
@languageCB
