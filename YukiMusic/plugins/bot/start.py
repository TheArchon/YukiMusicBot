...
from YukiMusic.misc import _boot_
from YukiMusic.plugins.sudo.sudoers import sudoers_list
...
@yuki.on_message(filters.command(["start"]) & filters.private & ~BANNED_USERS)
@LanguageStart
async def start_pm(client, message: Message, _):
    ...
    if name[0:3] == "inf":
        ...
        searched_text = _["start_6"].format(
            title, duration, views, published, channellink, channel, yuki.mention
        )
...
@yuki.on_message(filters.new_chat_members, group=-1)
            ...
            if member.id == yuki.id:
                ...
                if message.chat.id in await blacklisted_chats():
                    await message.reply_text(
                        _["start_5"].format(
                            yuki.mention,
                            f"https://t.me/{yuki.username}?start=sudolist",
                            config.SUPPORT_CHAT,
                        ),
