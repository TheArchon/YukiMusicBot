...
from YukiMusic.utils.formatters import alpha_to_int
from config import adminlist

IS_BROADCASTING = False


@yuki.on_message(filters.command("broadcast") & SUDOERS)
@language
async def braodcast_message(client, message, _):
    ...
    if "-nobot" not in message.text:
        ...
        for i in chats:
            try:
                m = (
                    await yuki.forward_messages(i, y, x)
                    if message.reply_to_message
                    else await yuki.send_message(i, text=query)
                )
