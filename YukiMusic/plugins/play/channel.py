...
from YukiMusic.utils.database import set_cmode
from YukiMusic.utils.decorators.admins import AdminActual
from config import BANNED_USERS


@yuki.on_message(filters.command(["channelplay"]) & filters.group & ~BANNED_USERS)
@AdminActual
async def playmode_(client, message: Message, _):
    ...
    if (str(query)).lower() == "disable":
        ...
        chat = await yuki.get_chat(message.chat.id)
        if chat.linked_chat:
        ...
        try:
            chat = await yuki.get_chat(query)
        except:
