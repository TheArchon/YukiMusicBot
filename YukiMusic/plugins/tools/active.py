...
from YukiMusic.utils.rich_stream import caption_blocks, edit_rich
...
def _build_caption(_, kind_label, chats, elapsed):
    if not chats:
        return _["ACTIVE_EMPTY"].format(kind_label, yuki.mention)
    ...
    return _["ACTIVE_LIST"].format(kind_label, len(chats), yuki.mention, body, elapsed)
...
@yuki.on_message(filters.command(["activevc", "activevoice", "ac"]) & SUDOERS)
async def activevc(client, message: Message):
    await _launch(message, "vc")


@yuki.on_message(filters.command(["activev", "activevideo"]) & SUDOERS)
async def activevi_(client, message: Message):
    await _launch(message, "video")


@yuki.on_callback_query(filters.regex(r"^ACTIVE ") & SUDOERS)
