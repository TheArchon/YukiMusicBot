...
from YukiMusic.utils import ShrutiBin, get_channeplayCB, seconds_to_min
from YukiMusic.utils.database import get_cmode, is_active_chat, is_music_playing
from YukiMusic.utils.decorators.language import language, languageCB
from YukiMusic.utils.inline import queue_back_markup, queue_markup
...
async def get_queue(client, message: Message, _):
    ...
    cap = _["queue_8"].format(yuki.mention, title, typo, user, send)
    upl = (
...
@yuki.on_callback_query(filters.regex("GetTimer") & ~BANNED_USERS)
...
@yuki.on_callback_query(filters.regex("GetQueued") & ~BANNED_USERS)
@languageCB
...
@yuki.on_callback_query(filters.regex("queue_back_timer") & ~BANNED_USERS)
@languageCB
