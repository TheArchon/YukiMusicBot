...
from YukiMusic.utils.database import get_assistant, get_authuser_names, get_cmode
from YukiMusic.utils.decorators import ActualAdminCB, AdminActual, language
from YukiMusic.utils.formatters import alpha_to_int, get_readable_time
...
async def reload_admin_cache(client, message: Message, _):
    try:
        ...
        async for user in yuki.get_chat_members(
...
@yuki.on_message(filters.command(["reboot"]) & filters.group & ~BANNED_USERS)
@AdminActual
async def restartbot(client, message: Message, _):
    mystic = await message.reply_text(_["reload_4"].format(yuki.mention))
    ...
    if chat_id:
        try:
            got = await yuki.get_chat(chat_id)
        except:
            pass
        ...
        try:
            db[chat_id] = []
            await Shruti.stop_stream_force(chat_id)
        except:
            pass
    return await mystic.edit_text(_["reload_5"].format(yuki.mention))


@yuki.on_callback_query(filters.regex("close") & ~BANNED_USERS)
...
@yuki.on_callback_query(filters.regex("stop_downloading") & ~BANNED_USERS)
@ActualAdminCB
