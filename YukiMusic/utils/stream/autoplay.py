...
from YukiMusic.platforms.Youtube import get_autoplay, is_external_path
from YukiMusic.utils.database import get_lang, is_autoplay
from YukiMusic.utils.formatters import seconds_to_min
from YukiMusic.utils.rich_stream import send_now_playing_rich
from YukiMusic.utils.stream.history import record_played, was_recently_played
from YukiMusic.utils.stream.queue import put_queue
...
async def try_autoplay(chat_id, popped) -> bool:
    ...
    run = await send_now_playing_rich(
        ...
        img,
        _["stream_1"].format(
            f"https://t.me/{yuki.username}?start=info_{next_id}",
            title[:23],
            duration_min,
            "Autoplay",
        ),
